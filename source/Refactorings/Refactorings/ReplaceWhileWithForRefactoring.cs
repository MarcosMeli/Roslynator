...
namespace Roslynator.CSharp.Refactorings
{
    internal static class ReplaceWhileWithForRefactoring
    {
        ...
        internal static async Task ComputeRefactoringAsync(RefactoringContext context, SelectedStatementsInfo info)
        {
            ...
            {
                StatementSyntax lastStatement = info.LastSelectedNode;

                if (lastStatement.IsKind(SyntaxKind.WhileStatement))
                {
                    IEnumerable<StatementSyntax> statements = info
                        .Nodes
                        .Skip(info.FirstSelectedNodeIndex)
                        .Take(info.LastSelectedNodeIndex - info.FirstSelectedNodeIndex);
                    ...
                    foreach (StatementSyntax statement in statements)
                    {
                        ...
                        if (kind == SyntaxKind.LocalDeclarationStatement)
                        {
                            ...
                            if (!IsAnySymbolReferenced(localDeclaration, info.Nodes, info.LastSelectedNodeIndex + 1, semanticModel, context.CancellationToken))
                            {
            ...
            if (localDeclarations != null)
            {
                ...
                if (StatementContainer.TryCreate(whileStatement, out container))
                {
                    ...
                    forStatement = forStatement
                        .TrimLeadingTrivia()
                        .PrependLeadingTrivia(localDeclarations[0].GetLeadingTrivia());
